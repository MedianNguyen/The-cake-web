...
public class ProductAction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("add".equals(request.getParameter("action"))) {
            request.getSession().setAttribute("AdminContent", "add");
            response.sendRedirect("/EprojectTheCake/Component/Admin/Admin.jsp");
            return;
        }
        int ID = Integer.parseInt(request.getParameter("ID"));
        Product p = null;
        ArrayList<Product> list = (ArrayList) request.getSession().getAttribute("product");
        for (Product po : list) {
            if (po.getId() == ID) {
                p = po;
            }
        ...
        request.getSession().setAttribute("productDetail", p);
